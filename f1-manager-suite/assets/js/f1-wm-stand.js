(function(){
  ...
  // robustes addEventListener (für Browser ohne Options-Objekt Support)
  function addEvt(el, type, fn, opts){
    if (!el || !el.addEventListener) return;
    try {
      el.addEventListener(type, fn, opts);
    } catch(e){
      // Fallback: nur capture boolean
      var capture = false;
  ...
  /* =========================
      Custom Dropdown (Session)
      - Fix: Menü als position:fixed im Viewport platzieren + "flip" (Opera GX Mobile etc.)
      ========================= */
  function initCustomDropdown(dd){
    ...
    function setActive(i){
      ...
      for (var j=0;j<opts.length;j++){
        opts[j].setAttribute("tabindex","-1");
      }
    ...
    // init: sicher geschlossen
    menu.hidden = true;
    menu.style.display = "none";
    resetMenuPosition();

    addEvt(btn, "click", function(e){
      e.preventDefault();
      toggle();
    });
    ...
    addEvt(btn, "keydown", function(e){
      ...
      if (key === "ArrowDown" || key === "Down"){
        e.preventDefault();
        if (!dd.classList.contains("is-open")) open();
        else setActive(Math.min(getSelectedIndex()+1, opts.length-1));
      } else if (key === "ArrowUp" || key === "Up"){
        e.preventDefault();
        if (!dd.classList.contains("is-open")) open();
        else setActive(Math.max(getSelectedIndex()-1, 0));
    ...
    addEvt(menu, "keydown", function(e){
      ...
      var activeIndex = 0;
      for (var i=0;i<opts.length;i++){
        if (opts[i].classList.contains("is-active")) { activeIndex = i; break; }
      }
    ...
    for (var idx=0; idx<opts.length; idx++){
      (function(i){
        ...
        addEvt(o, "click", function(e){
          e.preventDefault();
          selectIndex(i);
        });

        addEvt(o, "mouseenter", function(){
  ...
  /* =========================
      Mobile: Dropdown direkt NACH .entry-title in .entry-header verschieben
      ========================= */
    ...
    for (var i=0;i<dds.length;i++){
      ...
      if (mobile && header){
        ...
        if (!mount){
          ...
          // Mount direkt nach dem Titel einfügen
          if (title && title.parentNode === header){
            insertAfter(header, mount, title);
          } else {
            header.appendChild(mount);
          }
        } else {
          // Sicherstellen, dass mount wirklich direkt nach dem Titel sitzt
          if (title && title.parentNode === header){
            insertAfter(header, mount, title); // moves mount if already exists elsewhere
          }
        }

        if (dd.parentNode !== mount){
          forceClose(dd);
        ...
        if (info && info.parent && dd.parentNode !== info.parent){
          ...
          if (info.next && info.next.parentNode === info.parent){
            info.parent.insertBefore(dd, info.next);
          } else {
            info.parent.appendChild(dd);
          }
        }

        if (header){
          var mount2 = header.querySelector(".f1wms-entry-dd");
          if (mount2 && mount2.childNodes.length === 0){
            mount2.parentNode.removeChild(mount2);
          }
          if (!header.querySelector(".f1wms-entry-dd")){
            header.classList.remove("f1wms-entryhdr");
          }
        }
      }
    }
  ...
  ready(function(){
    ...
    for (var i=0;i<roots.length;i++){
      roots[i].classList.add("f1wms--js");
    }

    updateAllNameCols();
    if (document.fonts && document.fonts.ready) {
      document.fonts.ready.then(function(){ updateAllNameCols(); });
    }

    var dds = document.querySelectorAll("[data-f1wms-dd]");
    for (var j=0;j<dds.length;j++){
      initCustomDropdown(dds[j]);
    }
    ...
    for (var s=0;s<shells.length;s++){
      (function(shell){
        ...
        function maybeShow(){
          ...
          // Wenn der Scroller ausgeblendet ist (Mobile Card-View), kein Swipe-Hinweis
