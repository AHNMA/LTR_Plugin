(function(){
    ...
    const cfg = window.f1_tippspiel_cfg || {};
    const restRootRaw = (cfg.rest || '').toString();
    const nonce = cfg.nonce || '';
    const year = cfg.year;                // aktive Saison (vom Backend)
    const league_id = cfg.league_id;      // Default-Liga "gesamt" (vom Backend)
    const me_id = parseInt(cfg.me_id || '0', 10);

    const driverMap = {}; // driver_id => name
    const teamMap   = {}; // team_id => name
    ...
    async function loadMyStats(){
      ...
      if(!posTotal && !ptsTotal && !winsTotal){
        syncTopHeights();
        return;
      }
      ...
      // ✅ Siege (Backend-Feld kann je nach Version unterschiedlich heißen -> robust)
      if(winsTotal){
        let winsVal = null;
        if(meRow){
    ...
    function driverOptionsHtml(selectedId){
      ...
      if(!opts.length) return '<option value="">Fahrer wählen</option>';
      return opts.join('');
    }
    ...
    function teamOptionsHtml(selectedId){
      ...
      if(!opts.length) return '<option value="">Team wählen</option>';
      return opts.join('');
    }
    ...
    function renderBonusIntoRace(wrap, bonus){
      ...
      items.forEach(q=>{
        ...
        if(type === 'driver'){
          ...
          control = `<select class="f1tips-select" data-bonus-input="${id}" ${locked?'disabled':''}>
            ${driverOptionsHtml(sid)}
          </select>`;
        } else if(type === 'team'){
          ...
          control = `<select class="f1tips-select" data-bonus-input="${id}" ${locked?'disabled':''}>
            ${selectOptionsHtml(q.options||[], my)}
          </select>`;
        }
        ...
        if(String(q.status||'') === 'revealed' && correct){
          ...
          if(type === 'driver'){
            ...
            if(isNumericLike(my) && isNumericLike(correct)) right = (parseInt(my,10) === parseInt(correct,10));
              ...
              right = (myK === cK);
              if(!right && isNumericLike(correct)){
                right = (myK === normKey(nameOfTeam(parseInt(correct,10))));
              }
            }
          } else {
            right = (my && my === correct);
          ...
          infoInline = `<div class="f1tips-bonus-small">
            Lösung: <strong>${escapeHtml(correctLabel)}</strong>
            ${my ? (right ? ' · <strong>Richtig</strong>' : ' · <strong>Falsch</strong>') : ''}
            ${ptsText}
          </div>`;
        } else if(locked){
        ...
        html += `
          <div class="f1tips-card">
            <div class="f1tips-head">
              ...
              <div class="f1tips-bonus-meta">
                ${bonusStatusBadge(q)}
                <span class="f1tips-points">${points} P</span>
              </div>
            </div>
            ...
            <div class="f1tips-body">
              ...
              <div class="f1tips-bonus-actions">
                <button class="f1tips-btn f1tips-btn-primary"
                        ...
                        data-bonus-save="${id}"
                        ${locked ? 'disabled' : ''}
                        ${cfg.logged_in ? '' : 'disabled'}>
                  <span class="f1tips-btn-label">Antwort speichern</span>
                </button>

                <div class="f1tips-bonus-inline">
                  ${infoInline}
                </div>
    ...
    function updateWeekendPointsBoard(raceId, res){
      ...
      for(let i=1;i<=4;i++){
        ...
        if(cfg.logged_in && me_id && ended){
          const raw = (res && res.my_points && res.my_points[slug] != null)
            ? parseInt(res.my_points[slug], 10)
            : null;
    ...
    async function loadRaceData(raceId){
      ...
      // ✅ Session Badges + Buttons + ✅ Session-Auflösung (nur Punkte)
      // ✅ WICHTIG: "Ergebnis" NUR wenn Backend-State == ended (Beendet)
      ['quali','sq','sprint','race'].forEach(slug=>{
        ...
        // ✅ State kommt aus Backend (open|closed|ended|na)
        // Fallback: wenn states fehlt, dann wie früher über locked
        ...
        if(ended){
          state = 'result';
          label = 'Ergebnis';
        } else if(rawState === 'closed' || locked){
          state = 'closed';
          label = 'Geschlossen';
        }
        ...
        // ✅ "Was haben andere getippt?" nur sichtbar wenn:
        // - Session ist "Beendet" UND
        // - es gibt Tipps
        ...
        // ✅ Session-Punkte: nur wenn eingeloggt + Session ist "Beendet"
        ...
        if(myPtsLine){
          ...
          if(cfg.logged_in && me_id && ended){
            ...
            // auch 0 Punkte soll sichtbar sein
      ...
      if(res && res.bonus){
        renderBonusIntoRace(wrap, res.bonus);
      } else {
        const card = wrap.querySelector('[data-bonus-card]');
    ...
    function bindSaveButtons(){
      root.querySelectorAll('[data-save]').forEach(btn => {
        btn.addEventListener('click', async (e) => {
          e.preventDefault();
          if(!cfg.logged_in){
            toast('Bitte einloggen, um zu tippen.', false);
            return;
          }
          ...
          if(tip.length !== N || tip.some(v=>!v)){
            toast('Bitte alle Positionen ausfüllen.', false);
            return;
          }
    ...
    function bindBonusButtons(){
      root.addEventListener('click', async (e)=>{
        ...
        if(!cfg.logged_in){
          toast('Bitte einloggen, um Bonusfragen zu beantworten.', false);
          return;
        }
        ...
        if(!wrap){
          toast('Konnte Rennwochenende nicht bestimmen.', false);
          return;
        }

        const input = wrap.querySelector('[data-bonus-input="'+qid+'"]');
        if(!input){
          toast('Eingabefeld nicht gefunden.', false);
          return;
        }
        ...
        if(!answer){
          toast('Bitte eine Antwort auswählen/eingeben.', false);
          return;
        }
      ...
      root.querySelectorAll('[data-open-others]').forEach(btn=>{
        btn.addEventListener('click', async (e)=>{
          ...
          if(!payload || !payload.ok){
            if(othersBody) othersBody.innerHTML = '<div class="f1tips-note">Konnte Daten nicht laden.</div>';
            return;
          }
          ...
          if(!locked){
            html += '<div class="f1tips-note"><strong>Hinweis:</strong> Diese Session ist noch offen. Du siehst hier keine Tipps anderer.</div>';
          }

          if(Array.isArray(results) && results.length){
            html += `<div class="f1tips-card" style="box-shadow:none;margin-bottom:12px;">
              <div class="f1tips-head"><div class="title">Ergebnis</div></div>
              <div class="f1tips-body">${pillsFromIds(results)}</div>
            </div>`;
          }

          if(!tips.length){
            html += '<div class="f1tips-mut">Keine Tipps vorhanden.</div>';
            if(othersBody) othersBody.innerHTML = html;
            return;
          }

          html += `
            <div class="f1tips-list f1tips-list-others f1tips-list-scroll25">
              <div class="f1tips-list-head">
                <div>Spieler</div>
                <div>Punkte</div>
                <div>Tipps</div>
              </div>
              <div class="f1tips-list-body">
          `;
          ...
          tips.forEach(r=>{
            ...
            // ✅ IMMER " P" hinter Punkten
            const pts = (ptsRaw === null)
              ? '<span class="f1tips-mut">—</span>'
              : `<span class="f1tips-points">${ptsRaw} P</span>`;

            html += `
              <div class="f1tips-list-row">
                <div><strong>${escapeHtml(name)}</strong><div class="f1tips-mut">#${uid}</div></div>
                <div>${pts}</div>
                <div>${picks || '<span class="f1tips-mut">—</span>'}</div>
              </div>
            `;
          });
    ...
    (function initRaceDefault(){
      ...
      if(!rid){
        rid = parseInt((raceSelect.options[0] && raceSelect.options[0].value) || '0', 10) || 0;
      }
