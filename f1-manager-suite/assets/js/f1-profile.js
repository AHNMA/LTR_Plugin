(function(){
	...
	for (var f=0; f<forms.length; f++){
		(function(form){
			if(form.classList.contains('f1fp-js-disable-on-submit')){
	...
	if (avClick && fileInp && remInp) {
		avClick.addEventListener('click', function(){
			remInp.value = '0';
			fileInp.click();
		});
		fileInp.addEventListener('change', function(){
			if (fileInp.files && fileInp.files.length && avForm) {
				remInp.value = '0';
				disableForm(avForm);
				avForm.submit();
			}
