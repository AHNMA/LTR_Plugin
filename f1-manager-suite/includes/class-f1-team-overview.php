...
class F1_Team_Overview {
    ...
    public function enqueue_assets() {
        global $post;
        if ( is_a( $post, 'WP_Post' ) && has_shortcode( $post->post_content, 'f1_team_overview' ) ) {
            wp_enqueue_style( 'f1-team-overview', F1_MANAGER_SUITE_URL . 'assets/css/f1-team-overview.css', array(), '1.0.0' );
        }
    }

    public function render_shortcode( $atts ) {
        // Enqueue if not already (e.g. widget)
        wp_enqueue_style( 'f1-team-overview', F1_MANAGER_SUITE_URL . 'assets/css/f1-team-overview.css', array(), '1.0.0' );
        ...
        foreach ( $teams as $t ) {
            ...
            $row = array(
                ...
                'head'    => '#202020',
                'logo'    => self::get_teamlogo_url( isset($tm['teamlogo']) ? $tm['teamlogo'] : '' ),
                'car'     => self::get_carimg_url( isset($tm['carimg']) ? $tm['carimg'] : '' ),
                'socials' => self::render_socials( isset($tm['fb']) ? $tm['fb'] : '', isset($tm['x']) ? $tm['x'] : '', isset($tm['ig']) ? $tm['ig'] : '' ),
                'meta_rows' => array(),
                'drivers' => array(),
            );
            ...
            foreach ( $drvs as $d ) {
                ...
                $d_data = array(
                    ...
                    'img'    => self::get_driver_img_url( isset($dm['img']) ? $dm['img'] : '' ),
                    'flag'   => self::get_flag_url( isset($dm['flag']) ? $dm['flag'] : '' ),
                    'socials_std' => self::render_socials( isset($dm['fb']) ? $dm['fb'] : '', isset($dm['x']) ? $dm['x'] : '', isset($dm['ig']) ? $dm['ig'] : '' ),
                    'socials_head' => self::render_socials( isset($dm['fb']) ? $dm['fb'] : '', isset($dm['x']) ? $dm['x'] : '', isset($dm['ig']) ? $dm['ig'] : '', 'f1ov-driver-social--head' ),
                    'socials_below' => self::render_socials( isset($dm['fb']) ? $dm['fb'] : '', isset($dm['x']) ? $dm['x'] : '', isset($dm['ig']) ? $dm['ig'] : '', 'f1ov-driver-social--belowname' ),
                    'meta_rows' => array(),
                );
        ...
        ob_start();
        require F1_MANAGER_SUITE_PATH . 'includes/frontend/team-overview-view.php';
        return ob_get_clean();
    }
