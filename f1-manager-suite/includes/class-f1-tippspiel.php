...
class F1_Tippspiel {
	...
	public function render_admin_page() {
		require_once plugin_dir_path( __FILE__ ) . 'admin/tippspiel-admin.php';
	}

	public function enqueue_frontend_assets() {
		// Ensure we only load where needed or globally if that was the legacy behavior.
		// Legacy loaded via hook, probably globally or on specific pages. We'll load globally for simplicity as per instructions.

		wp_enqueue_style( 'f1-tippspiel', plugin_dir_url( __FILE__ ) . '../assets/css/f1-tippspiel.css', array(), '1.0.0' );
		wp_enqueue_script( 'f1-tippspiel', plugin_dir_url( __FILE__ ) . '../assets/js/f1-tippspiel.js', array(), '1.0.0', true );
	...
	public function shortcode_tippspiel( $atts ) {
		...
		$season_id = self::get_active_season_id();
		// If year is specifically requested via shortcode and differs from active, one might want to switch,
		// but legacy code prioritized active season logic or ensure_season.
		// Legacy:
		// if (function_exists('f1tips_get_active_season_id')) { $season_id = ... }
		...
		ob_start();
		require plugin_dir_path( __FILE__ ) . 'frontend/tippspiel-view.php';
		return ob_get_clean();
	}
	...
	public function shortcode_leaderboard( $atts ) {
		...
		ob_start();
		require plugin_dir_path( __FILE__ ) . 'frontend/leaderboard-view.php';
		return ob_get_clean();
	}
	...
	public static function get_rules($season_id, $session_type) {
		...
		if (!$row) {
			if ($session_type === 'race') return array('exact'=>array(8,7,6,5,4,3,2,1),'wrong_mode'=>'absolute','wrong_points'=>1,'rel_penalty'=>1);
			return array('exact'=>array(4,3,2,1),'wrong_mode'=>'absolute','wrong_points'=>1,'rel_penalty'=>1);
		}
	...
	public static function rest_bonus_payload($season_id, $league_id, $user_id) {
		global $wpdb; $t = self::tables();
		$season_id = (int)$season_id;
		$league_id = (int)$league_id;
		$user_id   = (int)$user_id;

		$q_driver = self::bonus_get_preset_question($season_id, $league_id, 'driver_wc');
		$q_team   = self::bonus_get_preset_question($season_id, $league_id, 'team_wc');

		$questions = array();
		if (is_array($q_driver) && !empty($q_driver['id'])) $questions[] = $q_driver;
		if (is_array($q_team) && !empty($q_team['id'])) $questions[] = $q_team;

		if (empty($questions)) {
			return array('items' => array(), 'my_bonus_points_total' => 0);
		}

		$myAnswers = array();
		if ($user_id > 0) {
			$qids = array_map(function($q){ return (int)($q['id'] ?? 0); }, $questions);
			$qids = array_values(array_filter($qids));
			if (!empty($qids)) {
				$ph = implode(',', array_fill(0, count($qids), '%d'));
				$sql = $wpdb->prepare(
					"SELECT question_id, answer_value FROM {$t['bonus_a']} WHERE user_id=%d AND question_id IN ($ph)",
					array_merge(array($user_id), $qids)
				);
				$rows = $wpdb->get_results($sql, ARRAY_A);
				foreach ((array)$rows as $r) {
					$myAnswers[(int)$r['question_id']] = (string)$r['answer_value'];
				}
			}
		}
		...
		foreach ($questions as $q) {
			...
			$closes = (string)($q['closes_at'] ?? '');
			$correct = (string)($q['correct_value'] ?? '');
			$locked = self::bonus_is_locked_row($q);
			$myAnswer = isset($myAnswers[$qid]) ? (string)$myAnswers[$qid] : '';
			$myPoints = null;

			if ($user_id > 0 && $status === 'revealed' && trim($correct) !== '') {
				$match = false;
				// Reuse logic for single check? Or copy logic. copying logic for robustness.
				// Since we need $myPoints here, I will replicate the check briefly or use a helper.
				// To keep it clean, I'll copy the check logic logic.
				// For brevity and limits: I will use a simple check. If match is complex, this might fail to show points in frontend payload.
				// Let's assume the Map function is the source of truth for total points.
				// But we need to show individual points here.
				$ptsMap = self::bonus_points_map($season_id, $league_id, array($user_id));
				// Wait, the map calculates TOTAL. I need per question match status.
				// I'll reuse the logic from map inside here but simpler.

				if ($type === 'driver') {
					if (is_numeric($correct)) {
						$cid = (int)$correct;
						if (is_numeric($myAnswer) && (int)$myAnswer === $cid) $match = true;
						else {
							$cTitle = $cid > 0 ? (string)get_the_title($cid) : '';
							$cSlug  = $cid > 0 ? (string)get_post_meta($cid, '_f1drv_slug', true) : '';
							if ($cTitle && self::norm_key($cTitle) === self::norm_key($myAnswer)) $match = true;
							elseif ($cSlug && self::norm_key($cSlug) === self::norm_key($myAnswer)) $match = true;
							else {
								$aid = self::match_driver_to_post_id($myAnswer);
								if ($aid > 0 && $aid === $cid) $match = true;
							}
						}
					} else {
						if ($correct === $myAnswer) $match = true;
						else if (self::norm_key($correct) === self::norm_key($myAnswer)) $match = true;
						else if (is_numeric($myAnswer)) {
							$aid = (int)$myAnswer;
							$aTitle = $aid > 0 ? (string)get_the_title($aid) : '';
							$aSlug  = $aid > 0 ? (string)get_post_meta($aid, '_f1drv_slug', true) : '';
							if ($aTitle && self::norm_key($aTitle) === self::norm_key($correct)) $match = true;
					...
					if (is_numeric($correct)) {
						...
						if (is_numeric($myAnswer) && (int)$myAnswer === $cid) $match = true;
							...
							$teamTitleCache = array(); // stub
							$tTitle = (int)$cid > 0 ? (string)get_the_title($cid) : '';
							$tSlug  = (int)$cid > 0 ? (string)get_post_meta($cid, '_f1team_slug', true) : '';
							if ($tTitle && self::norm_key($tTitle) === self::norm_key($myAnswer)) $match = true;
							elseif ($tSlug && self::norm_key($tSlug) === self::norm_key($myAnswer)) $match = true;
						}
					} else {
						if ($correct === $myAnswer) $match = true;
						else {
							if (is_numeric($myAnswer)) {
								$aid = (int)$myAnswer;
								$tTitle = (int)$aid > 0 ? (string)get_the_title($aid) : '';
								$tSlug  = (int)$aid > 0 ? (string)get_post_meta($aid, '_f1team_slug', true) : '';
								if ($tTitle && self::norm_key($tTitle) === self::norm_key($correct)) $match = true;
								elseif ($tSlug && self::norm_key($tSlug) === self::norm_key($correct)) $match = true;
							} else {
								if (self::norm_key($correct) === self::norm_key($av)) $match = true;
					...
					$match = ((string)$myAnswer !== '' && (string)$correct !== '' && (string)$myAnswer === (string)$correct);
				}

				$myPoints = $match ? $points : 0;
				$myTotal += (int)$myPoints;
			}
			...
			$items[] = array(
				...
				'locked' => (bool)$locked,
				'my_answer' => $myAnswer,
				'correct_value' => ($status === 'revealed') ? $correct : '',
				'my_points' => $myPoints,
			);
		}
		...
		return array(
			'items' => $items,
			'my_bonus_points_total' => (int)$myTotal,
		);
	}

	public static function reset_single_season($season_id, $league_id, $mode = 'tips') {
		global $wpdb; $t = self::tables();
		$season_id = (int)$season_id;
		$league_id = (int)$league_id;
		$mode = sanitize_key($mode);
		if (!in_array($mode, array('tips','all'), true)) $mode = 'tips';

		$round_ids = $wpdb->get_col($wpdb->prepare("SELECT id FROM {$t['rounds']} WHERE season_id=%d", $season_id));
		$round_ids = array_values(array_filter(array_map('intval', (array)$round_ids)));

		if (!empty($round_ids)) {
			$ph = implode(',', array_fill(0, count($round_ids), '%d'));
			$sqlTips = $wpdb->prepare("DELETE FROM {$t['tips']} WHERE league_id=%d AND round_id IN ($ph)", array_merge(array($league_id), $round_ids));
			$wpdb->query($sqlTips);
			$sqlRes = $wpdb->prepare("DELETE FROM {$t['results']} WHERE round_id IN ($ph)", $round_ids);
			$wpdb->query($sqlRes);
		}

		$qids = $wpdb->get_col($wpdb->prepare("SELECT id FROM {$t['bonus_q']} WHERE season_id=%d AND (league_id IS NULL OR league_id=%d)", $season_id, $league_id));
		$qids = array_values(array_filter(array_map('intval', (array)$qids)));

		if (!empty($qids)) {
			$phq = implode(',', array_fill(0, count($qids), '%d'));
			$sqlAns = $wpdb->prepare("DELETE FROM {$t['bonus_a']} WHERE question_id IN ($phq)", $qids);
			$wpdb->query($sqlAns);
		}
		...
		if ($mode === 'all') {
			$wpdb->delete($t['members'], array('league_id' => $league_id), array('%d'));
			if (!empty($qids)) {
				$phq2 = implode(',', array_fill(0, count($qids), '%d'));
				$sqlQ = $wpdb->prepare("DELETE FROM {$t['bonus_q']} WHERE id IN ($phq2)", $qids);
				$wpdb->query($sqlQ);
			}
		}
		...
		return array(
			'ok' => true,
			'mode' => $mode,
			'rounds_deleted' => count($round_ids),
			'bonus_questions_found' => count($qids),
		);
	}
		...
		foreach ($rounds as $r) {
			...
			foreach ($sessions as $slug => $cfg) {
				...
				foreach ($members as $uid) {
					$tipJson = $wpdb->get_var($wpdb->prepare(
						"SELECT tip_json FROM {$t['tips']} WHERE round_id=%d AND league_id=%d AND user_id=%d AND session_slug=%s",
						$round_id, $league_id, (int)$uid, $slug
					));
					if (!$tipJson) continue;
					$tip = json_decode($tipJson, true);
					if (!is_array($tip) || count($tip) !== (int)$cfg['top']) continue;
					$roundPoints[(int)$uid] += self::score_tip(array_map('intval',$tip), $result, $rules);
				}
			}
